...
| 49 | Wall off depth with no legible next step, forcing in-area grinding | Developer playthrough: an early tool-upgrade gate (around 20 m) read as a hard wall and forced mining the current area to afford it; One Man's Trash reviewers also resented the ~$20,000 license gate | Capability gates are fine only when they read as a visible next step: the player sees what they cannot yet bite and returns with power, never a wall without an answer (`03` §5, `06` §1) |
