...
Entries combine reviewed player complaints, developer preferences and design inferences. The rule
column states what this game adopts; an example is not evidence of a universal player preference. If a proposal matches an entry, it is rejected unless the developer explicitly
...
|---|---|---|---|
| 1 | Let players max everything halfway and make money pointless | "completely max out everything before the hole is even 15% of the way down" / "ended with 100k and nowhere to spend" | Meaningful upgrades through the campaign, useful remaining play after the final purchase; a fully completed finite economy may end (`06`) |
...
| 3 | Sell invisible stat upgrades | "+5% range, +5% power… you don't feel the effect"; Ore Factory Squad's reviewers finished it feeling upgrades never increased their output | Fewer stronger steps; practical benefit on the next outing, not cosmetic feedback alone (`04`, `06`) |
...
| 6 | Force repeat trips or grinds for the finale | "40 to 60 more trips to the bottom" | Discoverable component trails; owned parts insert without retrieval trips; no grind wall (`05`, `11`) |
...
| 44 | Gate progression behind a minigame or non-digging activity | Hydroneer's King's quest demanded token crops, a 1.5-weight lobster and 600-crop soup deliveries — "quest grind fest" | Every zone stays diggable with current equipment; money buys power and convenience, never a compulsory depth gate (`03`, `06`) |
| 47 | Ship an upgrade that is bigger but slower in the ground it replaces | Ore Factory Squad's jackhammer was faster per hit but had a smaller bite than the upgraded shovel — players called it "really useless", and its tool tiers read as "more or less the same" | Every tier improves familiar-ground output; power outpaces new ground resistance and material responses stay distinct (`04`, `06`) |
...
| 49 | Wall off depth and force in-area grinding | Developer playthrough: an early tool-upgrade gate (around 20 m) read as a hard wall and forced mining the current area to afford it; One Man's Trash reviewers also resented the ~$20,000 license gate | No mandatory depth/tool-power walls, even when labeled clearly; optional pockets always yield visible progress to the current tool (`03`, `06`) |
...
| 10 | Punish falls with health/chip damage | "fall damage is not gracious"; 1-voxel specks break legs | Harmless landing feedback; no battery loss, input lock or ordinary-fall recovery (`04`, `10`) |
...
| 12 | Make darkness pitch black | "extreme darkness" complaints | Ground and object shapes always readable; lamps improve detail and mood (`03`) |
...
| 15 | Leave floating specks that snag movement | "1x1-pixel particles that will break your legs"; Ore Factory Squad's reviewers specifically praised that its digging left no floating bits — clean cleanup is a selling point | Plain crumbs vanish; valuables collect only with space, overflow persists nonblocking; interesting objects remain (`03`, `05`) |
...
| 19 | Depth-only value with nothing sideways | Straight-down trivialization; Ore Factory Squad tied worth strictly to depth and ended with top-tier floods while its basic inputs went scarce | Fixed price per type; deeper zones can contain richer types; coherent scenes and connected finds reward sideways digging (`02`, `03`) |
...
| 34 | A permanent tutorial or popup spam | tutorial complaints; Ore Factory Squad's forced, unskippable tutorial that could not be saved through caused restarts before play began | No compulsory tutorial; world labels, contextual action prompts and optional controls reference (`07`) |
...
| 38 | Lock achievements behind NG+, RNG or bugs | "INCREDIBLY scummy"; 45/46 forever | All required content in every seed; fair one-run Steam achievements separate from current-save completion (`12`) |
| 39 | Delete or invalidate the display / collection | progress loss | Special exhibits persist; any compatible display arrangement counts (`07`) |
...
| 42 | Block the main thread during save serialization or freeze the game on save | Digger: Galactic Treasures PC freezes for 15–60s on every save due to synchronous whole-world voxel serialization | Nonblocking saves preserve input and dig rhythm; frame-impact and total background-save duration are different measurements (`08`, `14`) |
