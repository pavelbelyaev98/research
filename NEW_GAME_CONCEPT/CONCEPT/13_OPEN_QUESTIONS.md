...
|---|---|---|---|
| D03 | What are distinctive finds ultimately for? | First copy to display with duplicates sold / sell-only with snapshot / notable-signal-only split | Deferred by developer. Principle fixed: **no keep/sell choice per item**; each type has one fixed purpose. Recommendation on record: first copy displays, duplicates sold; a notable-distinctives-only split is the bounded middle path. Ore Factory Squad's special collectibles read as pointless because their only interaction was a small fixed sale with no record — a find tier without a home feels like vendor trash, which supports first-copy-to-display |
| D14 | When does a unique's one-sentence story appear? | On pickup / on inspection / both | Deferred; the story itself is confirmed (D14). Playthrough note: inspection is never required for commons; pickup delivery with optional inspection fits the developer's own play |
...
| Q32 | Social hooks for a solo game (netcode-free) | None / seed and hole-code sharing / display-wall snapshots or postcards / friend records and depth boards / co-op later | Research-derived: Keep Digging's most-praised feature is co-op (its top reviews are friend groups); co-op requests also appear in AGADAH and The Spotter, and Ore Factory Squad's co-op is its most-praised mode too. Q24 is re-opened for decisioning; if co-op stays out, decide whether any cheap social hook is wanted. Cheapest candidate: a photo-mode postcard export (seed code, hole depth/shape, favourite find) — offline, shareable, and impossible to mistake for online play |
...
| S15 | Exploit policy for duplication/value loopholes | Fix every value exploit / allow harmless physics comedy only / embrace all as community culture | Recommendation: any exploit that creates money or skips a purchase is fixed; purely physical comedy that cannot move value may stay; by construction, value never grows by combining (`06` §8). Construction rules: purchased objects have no resale value, no free-to-create item is sellable, and placed devices do not refund at purchase price — Ore Factory Squad's full-refund device and free-spawnable logistics items let players skip the loop, while its explosive and physics comedy was harmless and celebrated |
| S16 | Do timed/temporary power-ups have a place (burst items, overcharge)? | None / temporary boost items / timed overcharge after big finds | Parked: the corpus has no signal. Developer playthrough liked timed boosts in One Man's Trash but suspects the boredom flattered them; revisit only if a playtest shows the loop needs a mid-run spike |
...
| ID | Question | Current options | Recommendation / note |
|---|---|---|---|
| W01 | Site footprint (depth floor fixed at ≥100 m) | Compact vertical / wider / other | Exact depth and footprint set by feel in playtest; keep the footprint contained. Ore Factory Squad's players complained their dig plots were tiny and bottomed out within minutes — each zone's lateral room must not feel exhausted at a glance |
...
| F07 | Detector presentation | Hybrid tool glow + edge hint / edge indicator only / tool reaction only | Deferred; options documented; decide with a prototype and review research. Ore Factory Squad's ore-discovery popups fired constantly while digging and were called an annoyance — discovery never announces itself with a toast |
| F10 | UI art style | Industrial-worksite / other | Clear, not retro; exact style later |
| W13 | Return-path navigation in a free-form hole (supports Q27's no-return-aids rule) | Current mitigations only (sky-lit shaft, lamps, depth readout, detector) / terrain repack or "spit dirt" ability / route-marker tool / consumable ascent aid | getting lost is one of Meltopia's top complaints (no map, tunnels all alike); One Man's Trash reviewers praised rebuilding staircases with spat-out dirt as the best QoL over AGADAH; Keep Digging players used save-and-exit as an elevator. Requirement on record: legibility, not a map widget. Recommend a return-navigation prototype metric and a terrain-repack spike. Prototype/release gates now in `14_PROTOTYPE_PLAN.md` §4/§7 |
| W14 | Non-cave underground structures and passages | Authored zones only (current) / authored non-cave structures inside zones / procedural passages | Developer playthrough: generated non-cave passages were interesting but may push the game in the wrong direction. Keep the authored approach unless a prototype shows they add discovery without diluting the fantasy |
| F13 | Performance-induced hitch feel (shader compilation, terrain streaming, late-session FPS decay) | Treat as a feel requirement with a prototype budget (cold-start hitch, stable dig rhythm) / accept / dedicated spike | Biggest cross-corpus negative: Keep Digging players report frame collapses and shader stutter; Meltopia players report freezes and hitches; Digger: Galactic Treasures serialized its voxel world synchronously and froze during saves. Production targets stay parked; this is about feel and recognition rhythm. Rule on record: optimization work must never alter existing saves' behavior. Prototype/release gates now in `14_PROTOTYPE_PLAN.md` §4/§7 |
