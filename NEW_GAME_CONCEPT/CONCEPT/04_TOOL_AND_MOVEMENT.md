...
- **Power escalation:** the machine grows faster than the ground gets tougher (S17). Small early
...
- **Additional auto-dig assist** stays for implementation if it offers something beyond hold/toggle (I10).
...
There is no mode button and no required switching (S09). The machine reads the ground and changes
...
- **Soft preference, never a lock:** each material family clearly rewards one behavior (sand rewards
 ...
 behavior can dig everything (W05, S09). Visual/audio feedback shows the response the machine
...
- **No late convergence:** the final head keeps distinct automatic material responses; it does not
 turn every material into the same vacuum action (S09).
- **Upgrades improve all behaviors at once** — one shared tool upgrade level (Q30b). No separate
 upgrade economy for a second tool.
- **Seam digging** uses these same automatic responses: aim along a visible seam for a larger,
 more efficient local cut (W16; `03_WORLD_AND_SITE.md`). No extra mode or required technique.

## 4. Upgrade tracks and the tool

The tool's own track (Tool) controls power, bite size and adaptation quality; see
`06_PROGRESSION_AND_ECONOMY.md` for all six tracks. Fewer, stronger steps replace tiny increments;
each purchase noticeably improves the next outing and applies without a blocking animation (S02).

## 5. Jetpack

- Starts simple and **stable**; never deliberately hard to control (S07).
...
- **Thrown or placed, then remotely detonated.** Multiple charges can be active at once (S08).
...
- Charges cost money; C4 is an optional accelerator, never the only way past anything (W05).
